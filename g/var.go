...
func SendToZabbix(metrics []*model.MetricValue) {
	if len(metrics) == 0 {
		return
	}
	...
	if debug {
		log.Printf("zabbix=> <Total=%d> %v\n", len(metrics), metrics[0])
	}
	SendZabbixMetrics(metrics)
}

func SendToKafka(metrics []*model.MetricValue) {
	if len(metrics) == 0 {
		return
	}
	...
	if debug {
		log.Printf("kafka=> <Total=%d> %v\n", len(metrics), metrics[0])
	}
	SendKafkaMetrics(metrics)
}
